...
# ---------------------------------------------------------------------------
# Cast of characters -> LaTeX (back matter)
# ---------------------------------------------------------------------------

def render_cast_person(p: dict) -> str:
    """Render one cast entry: name, relation, bio, and the letters they appear in."""
    name = latex_escape(p.get("name", ""))
    lines = []
    head = f"\\noindent{{\\displayfont\\itshape\\large {name}}}"
    if p.get("uncertain"):
        head += "\\,{\\color{brass}\\textsuperscript{?}}"
    lines.append(head + "\\par")
    relation = p.get("relation", "")
    if relation:
        lines.append(f"\\nopagebreak{{\\eyebrow{{{latex_escape(relation)}}}}}\\par")
    lines.append("\\nopagebreak\\vspace{0.2em}")
    bio = p.get("bio", "")
    if bio:
        lines.append(f"\\nopagebreak{{\\small {finalize_bold(latex_escape(bio))}}}\\par")
    letters = p.get("letters") or []
    if letters:
        ref = ", ".join(latex_escape(x) for x in letters)
        n = len(letters)
        word = "letter" if n == 1 else "letters"
        lines.append(
            f"\\nopagebreak{{\\monofont\\footnotesize\\color{{walnutmute}}"
            f" Appears in {n} {word}: {ref}\\par}}"
        )
    lines.append("\\smallskip")
    return "\n".join(lines)


def render_cast_group(group: dict, members: list[dict]) -> str:
    out = ["\\bigskip"]
    out.append(f"{{\\centering\\eyebrow{{{latex_escape(group['label'])}}}\\par}}")
    out.append("\\brassrule[28pt]")
    if group.get("blurb"):
        out.append(
            f"{{\\centering\\small\\itshape\\color{{walnutmute}} "
            f"{latex_escape(group['blurb'])}\\par}}"
        )
    out.append("\\medskip")
    out.extend(render_cast_person(p) for p in members)
    return "\n".join(out)


def build_cast(repo: Path, out_dir: Path) -> None:
    """Emit generated/cast.tex from <repo>/cast.json (the same source the web uses)."""
    cast_path = repo / "cast.json"
    out_path = out_dir / "cast.tex"
    if not cast_path.exists():
        out_path.write_text(
            "% cast.json not found; run the cast-extraction step and "
            "scripts/build_cast.py first.\n",
            encoding="utf-8",
        )
        sys.stderr.write("-- cast.json not found; wrote empty generated/cast.tex\n")
        return
    cast = json.loads(cast_path.read_text(encoding="utf-8"))
    people = cast.get("people", [])
    blocks = []
    for group in cast.get("groups", []):
        members = [p for p in people if p.get("group") == group["key"]]
        members.sort(key=lambda p: (p.get("sort") or p.get("name") or "").lower())
        if not members:
            continue
        blocks.append(render_cast_group(group, members))
    out_path.write_text(
    ...
    sys.stderr.write(f"Wrote cast: {len(people)} people -> {out_path.name}\n")
...
def main() -> int:
    ...
    # Cast of characters (back matter), generated from the shared cast.json.
    build_cast(repo, out_dir)
